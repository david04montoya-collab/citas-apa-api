...
def buscar_google_scholar_apa_especifico(terminos, max_results=2):
    ...
    return citas
        ...
        for art in resultados[:max_results]:
            try:
                title = art.get("title", "").strip()
                ...
                # Formato APA
                cita_apa = f"{autor_apa} ({año}). {title}. Recuperado de {link}"
                citas.append(cita_apa)
        ...
        print(f"Error en buscar_google_scholar_apa: {e}")
